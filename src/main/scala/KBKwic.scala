...
object KBKwic
{
  ...
  def concordance(query:TextQuery, document:Node):List[Kwic] = 
  {
    val tokens = tokenize(document.text)
  ...
  def concordance(query:TextQuery, url:String):List[Kwic] = concordance(query,XML.load(url))
  ...
  def kwicResultsPar(t:TextQuery)
  {
      ...
      split.par.foreach(
           x =>  
             ...
             { println(KBKwic.concordance(t, id)) }
      )
  }
