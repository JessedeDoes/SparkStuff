...
import hilex.{Attestation, Lemma, Sense, hilexQueries}
import wsd.{ConvertOldInstanceBase, DistributionalOnly, Swsd, tester}

/**
  * Created by jesse on 6/6/17.
  */
import concordance.Concordance
import hilex.Hilex._
...
object  DictionaryWSD
{
  ...
  {
    val c:Concordance = a.toConcordance
  ...
  def testWSD(lemmaId: String) =
  {
    ...
    val l:List[Lemma] = slurp(hilexQueries.lemmaQueryWhere(s"persistent_id='${lemmaId}'"))
    val theLemma = l.head
    println(theLemma)
